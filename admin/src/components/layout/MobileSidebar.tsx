    ...
    <div className="fixed inset-0 z-50 flex lg:hidden bg-slate-900/40 backdrop-blur-sm animate-in fade-in">
      <div className="relative flex w-4/5 max-w-xs flex-col border-r border-slate-200 bg-white p-5 shadow-2xl">
        <div className="flex items-center justify-between border-b border-slate-200/80 pb-4">
          ...
          <button
            onClick={onClose}
            className="rounded-md p-1.5 text-slate-400 hover:bg-slate-100 hover:text-slate-900 transition"
          >
            <X className="h-5 w-5" />
          </button>
        </div>
          ...
          {navigationItems.map((item) => {
            ...
            const linkContent = (
              <Link
                ...
                href={item.href}
                onClick={onClose}
                className={`flex items-center rounded-md px-3.5 py-2.5 text-xs font-medium transition-colors duration-150 ${
                  isActive
